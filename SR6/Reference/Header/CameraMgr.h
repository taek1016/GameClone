...
private:
	explicit						CCameraMgr(void) = default;
	virtual							~CCameraMgr(void);

public:
	const _int						Get_CurCamera(void) { return m_iCurCamera; }
	const _vec3*					Get_At() const { return m_vecCamera[m_iCurCamera]->Get_At(); }
	const _vec3*					Get_Eye() const { return m_vecCamera[m_iCurCamera]->Get_Eye(); }
	
	const _matrix*					Get_ViewSpace(void) { return m_vecCamera[m_iCurCamera]->Get_ViewSpace(); }
	const _matrix*					Get_Projection(void) { return m_vecCamera[m_iCurCamera]->Get_Projection(); }
