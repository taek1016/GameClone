	...
	LPD3DXMESHCONTAINER *ppNewMeshContainer)
{
	...
	if (0 != NumMaterials)
	{
		...
		for (_ulong i = 0; i < pDerivedMeshContainer->NumMaterials; ++i)
		{
			...
			if (nullptr == pDerivedMeshContainer->pMaterials[i].pTextureFilename)
			{
				continue;
			}
			...
			MultiByteToWideChar(CP_ACP,
				...
				static_cast<_int>(strlen(pDerivedMeshContainer->pMaterials[i].pTextureFilename)),
				szFileName,
				256);
...
STDMETHODIMP Engine::CHierarchyLoader::DestroyMeshContainer(THIS_ LPD3DXMESHCONTAINER pMeshContainerToFree)
{
	...
	if (1 == pMeshContainer->NumMaterials)
	{
		return S_OK;
	}
	...
	{
		if (nullptr != pMeshContainer->ppTexture[i])
		{
			if (0 == Safe_Release(pMeshContainer->ppTexture[i]))
			{
...
void CHierarchyLoader::Allocate_Name(char ** ppName, const char * pFrameName)
{
	...
	_uint		iLength = static_cast<_uint>(strlen(pFrameName));
